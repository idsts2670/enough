...
describe('Plaid suggested categories', () => {
  afterEach(() => {
    delete handlers['/plaid/transactions'];
  });

  test('creates a Plaid category when a suggested category does not exist', async () => {
    const categoryIdsByName = new Map();

    const categoryId = await getOrCreatePlaidSuggestedCategory(
      'Food & Drink',
      -12.34,
      categoryIdsByName,
    );
    ...
    expect(group.name).toBe('Plaid Categories');
    expect(group.is_income).toBe(0);
    expect(categoryIdsByName.get('food & drink')).toBe(categoryId);
  });

  test('creates an income category for positive Plaid income suggestions', async () => {
    const categoryIdsByName = new Map();
    ...
    expect(category.is_income).toBe(1);
    expect(group.name).toBe('Plaid Income');
    expect(group.is_income).toBe(1);
  });

  test('assigns Plaid suggested categories during account sync', async () => {
        ...
        all: [
          {
            ...
            transactionAmount: { amount: '-4.50' },
            booked: true,
            plaidSuggestedCategory: 'Food & Drink',
          },
        ],
        booked: [],
        pending: [],
    ...
    await db.insertAccount({
      id: 'plaid-account',
      account_id: 'plaid-source-account',
      name: 'Plaid Account',
      account_sync_source: 'plaid',
    });
    await db.insertPayee({
      id: 'transfer-plaid-account',
      name: '',
      transfer_acct: 'plaid-account',
    });
    ...
    await syncAccount(
      ...
      undefined,
      'plaid-account',
      'plaid-source-account',
      'plaid-bank',
      '2026-01-01',
    );

    const transactions = await getAllTransactions();
    const imported = transactions.find(
      transaction => transaction.imported_id === 'plaid-txn-1',
    );
    const category = await db.first<db.DbCategory>(
      'SELECT * FROM categories WHERE id = ?',
      [imported.category],
  ...
  test('backfills uncategorized Plaid transactions from raw sync data', async () => {
    vi.mocked(asyncStorage.getItem).mockResolvedValue('test-token');
    handlers['/plaid/transactions'] = () => ({
      transactions: {
        all: [],
    ...
    const payeeId = await db.insertPayee({ name: 'Coffee Shop' });
    await db.insertTransaction({
      id: 'existing-plaid-transaction',
