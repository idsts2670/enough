...
const PLAID_EXPENSE_CATEGORY_GROUP = 'Plaid Categories';
const PLAID_INCOME_CATEGORY_GROUP = 'Plaid Income';
const IGNORED_PLAID_SUGGESTED_CATEGORIES = new Set(['uncategorized']);

async function getOrCreateCategoryGroup(
  name: string,
  isIncome: boolean,
): Promise<db.DbCategoryGroup['id']> {
  const existingGroup = await db.first<Pick<db.DbCategoryGroup, 'id'>>(
    'SELECT id FROM category_groups WHERE UNICODE_LOWER(name) = ? AND tombstone = 0 LIMIT 1',
    [name.toLowerCase()],
  );
...
export async function getOrCreatePlaidSuggestedCategory(
  suggestedCategory: string | null | undefined,
  amount: number | string | null | undefined,
  categoryIdsByName: Map<string, db.DbCategory['id']>,
): Promise<db.DbCategory['id'] | null> {
  if (!suggestedCategory) {
    return null;
  }

  const normalizedName = suggestedCategory.trim().toLowerCase();
  if (
    !normalizedName ||
    IGNORED_PLAID_SUGGESTED_CATEGORIES.has(normalizedName)
  ) {
    return null;
  }

  const existingCategoryId = categoryIdsByName.get(normalizedName);
  if (existingCategoryId) {
    return existingCategoryId;
  }

  const isIncome = normalizedName === 'income' && Number(amount ?? 0) > 0;
  const categoryGroupId = await getOrCreateCategoryGroup(
    isIncome ? PLAID_INCOME_CATEGORY_GROUP : PLAID_EXPENSE_CATEGORY_GROUP,
    isIncome,
  );
  ...
  categoryIdsByName.set(normalizedName, categoryId);
  return categoryId;
}
...
async function normalizeBankSyncTransactions(transactions, acctId) {
  ...
  for (const trans of transactions) {
    ...
    const suggestedCategory = await getOrCreatePlaidSuggestedCategory(
      trans.plaidSuggestedCategory,
      trans.amount,
      categoryIdsByName,
    );
    ...
    normalized.push({
      ...
      trans: {
        ...
        category: trans.category ?? suggestedCategory ?? null,
        imported_id,
...
async function backfillPlaidSuggestedCategories(acctId: AccountEntity['id']) {
  const transactions = await db.all<
    Pick<TransactionEntity, 'id' | 'amount' | 'category' | 'raw_synced_data'>
  >(
    `SELECT id, amount, category, raw_synced_data
       FROM v_transactions_internal
      WHERE account = ?
        AND tombstone = 0
        AND is_parent = 0
        AND category IS NULL
        AND raw_synced_data IS NOT NULL`,
    [acctId],
  );
  ...
  for (const trans of transactions) {
    let rawSyncedData;
    try {
      rawSyncedData = JSON.parse(trans.raw_synced_data);
    } catch {
      continue;
    }

    const category = await getOrCreatePlaidSuggestedCategory(
      rawSyncedData?.plaidSuggestedCategory,
      trans.amount,
      categoryIdsByName,
    );

    if (category) {
      updated.push({ id: trans.id, category });
    }
  }
  ...
  if (initialSync) {
    ...
    return runMutator(async () => {
      ...
      if (acctRow.account_sync_source === 'plaid') {
        await backfillPlaidSuggestedCategories(id);
      }
      return {
        ...result,
