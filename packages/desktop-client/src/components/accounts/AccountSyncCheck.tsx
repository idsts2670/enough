...
function useErrorMessage() {
  ...
  function getErrorMessage(type: string, code: string) {
    switch (type.toUpperCase()) {
      ...
      case 'INVALID_ACCESS_TOKEN':
        return t('This bank connection is no longer authorized.');

      case 'ACCOUNT_NEEDS_ATTENTION':
        return t('This bank connection needs your attention.');

      case 'ACCOUNT_MISSING':
        return t('This account was not found. Try unlinking and relinking it.');
...
export function AccountSyncCheck() {
  ...
  const reauth = useCallback(
    (acc: AccountEntity) => {
      ...
      if (acc.account_sync_source === 'plaid' && acc.account_id) {
        const itemId = acc.account_id.split('::')[0];
        dispatch(
          pushModal({
            modal: {
              name: 'plaid-link',
  ...
  const showAuth =
    account.account_sync_source === 'plaid' &&
