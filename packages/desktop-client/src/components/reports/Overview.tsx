...
function isDefaultDashboardTipsWidget(widget: DashboardWidgetEntity) {
  return (
    widget.type === 'markdown-card' &&
    widget.meta.content.trim().startsWith('## Dashboard Tips')
  );
}
...
export function Overview({ dashboard }: OverviewProps) {
  ...
  const visibleWidgets = useMemo(
    () => widgets.filter(shouldShowDashboardWidget),
    [widgets],
  );
  ...
  const mobileLayout = useMemo(() => {
    ...
    const sortedDesktopItems = [...visibleWidgets];
  ...
  const widgetMap = useMemo(
    () => new Map(visibleWidgets.map(widget => [widget.id, widget])),
    [visibleWidgets],
  );
