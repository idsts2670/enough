...
export function ImportActualModal() {
  ...
  function getErrorMessage(error: string): string {
    switch (error) {
      ...
      case 'not-zip-file':
        return t(
          'This file is not valid. Please select an unencrypted archive of Enough data.',
        );
      case 'invalid-zip-file':
        return t('This archive is not a valid Enough export file.');
  ...
  return (
    <Modal name="import-actual" containerProps={{ style: { width: 400 } }}>
      {({ state }) => (
        <>
          <ModalHeader
            title={t('Import from Enough export')}
          ...
          <View style={{ ...styles.smallText, lineHeight: 1.5, marginTop: 20 }}>
            ...
            <View style={{ '& > div': { lineHeight: '1.7em' } }}>
              <Paragraph>
                <Trans>
                  You can import data from another Enough account or instance.
